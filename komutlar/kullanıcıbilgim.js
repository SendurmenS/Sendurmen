...
exports.run = (client, message, params) => {
    if (message.channel.type !== "group") {
      ...
      .setColor(Durm)
      .setTimestamp()
      .addField('Ad (İsim):', message.author.username + '#' + message.author.discriminator)
      .addField('ID:', message.author.id)
      .addField('Discord Kayıt tarihi:', message.author.createdAt)
      .addField('Durum:', durm)
      .addField('Şu an oynadığı oyun:', message.author.presence.game ? message.author.presence.game.name : 'Şu an oyun oynamıyor')
      .addField('Botmu İnsanmı?', message.author.bot ? '\nBOT' : 'İnsan')
